...
export function renderPlan(store: Store): string {
  ...
  dto.lanes.forEach((lane, idx) => {
    ...
    for (const it of lane.items) {
      const name = c.bold(it.stage.name);
      const deleg = it.is_delegation_candidate ? c.cyan("  ⇄ delegate") : "";
      const why = it.rationale ? c.dim(`  — ${it.rationale}`) : "";
      lines.push(
        `  ${scheduledBadge(it.scheduled_state)}  ${name} ${c.dim(`(#${it.task.id} ${it.task.title})`)}${dueBadge(it.dueBand)}${deleg}${why}`,
      );
    }
  });
  return lines.join("\n");
